...
namespace Erinn
{
    ...
    ///     Network server
    /// </summary>
    public sealed class ENetPunchService : NetworkServerPeer
    {
        /// <summary>
        ///     Server EndPoint
        /// </summary>
        ...
        ///     EndPoints
        /// </summary>
        private readonly ConcurrentMap<uint, ENetPunchEndPoint> _endPoints = new();

        /// <summary>
        ///     Event queue
        /// </summary>
        private readonly ConcurrentQueue<ENetPunchServiceEvent> _onryoEvents = new();
        ...
        ///     Structure
        /// </summary>
        public ENetPunchService() : base(new NetworkServerMessageChannel())
        {
        }
        ...
        public ENetPunchService(NetworkServerMessageChannel messageChannel) : base(messageChannel)
        {
        }
        ...
        private void PublishEvent(in ENetPunchServiceEvent onryoEvent) => _onryoEvents.Enqueue(onryoEvent);
            ...
            while (!polled)
            {
                ...
                switch (netEvent.Type)
                {
                    case EventType.Receive:
                        ...
                        PublishEvent(new ENetPunchServiceEvent(NetworkEventType.Data, id, networkPacket));
                        continue;
                    case EventType.Connect:
                        ...
                        if ((netEvent.Data != 3847762548U && netEvent.Data != 1139550914U) || !IPAddress.TryParse(ip, out var ipAddress))
                        {
                            AddToBlacklist(ip);
                            peer.DisconnectNow(0U);
                            continue;
                        ...
                        peer.PingInterval(6000U);
                        peer.Timeout(0U, 60000U, 60000U);
                        _pendingConnections.Add(id);
                        PublishEvent(new ENetPunchServiceEvent(NetworkEventType.Connect, id, netEvent.Data == 1139550914U, new IPEndPoint(ipAddress, peer.Port), peer));
                        continue;
                    ...
                    default:
                        ...
                        {
                            _endPoints.TryRemove(id, out _);
                            PublishEvent(new ENetPunchServiceEvent(NetworkEventType.Disconnect, id));
                            continue;
                        }
            ...
            while (_onryoEvents.TryDequeue(out var netEvent))
            {
                ...
                switch (netEvent.EventType)
                {
                    ...
                    case NetworkEventType.Connect:
                        ...
                        var endPoint = new ENetPunchEndPoint(netEvent.RemoteEndPoint);
                        _endPoints[id] = endPoint;
                        if (netEvent.IsServer && ENetSerializer.Create(in endPoint, out var packet))
                            _outgoing.Enqueue(new ENetPacket(netEvent.Connection, packet));
        ...
        {
            if (!_endPoints.TryGetValue(id, out var from) || (from.Address == endPoint.Address && from.Port == endPoint.Port) || !_endPoints.TryGetKey(endPoint, out var toId))
                return;
            var message = from.Create();
            Send(toId, in message);
        }
    }
}