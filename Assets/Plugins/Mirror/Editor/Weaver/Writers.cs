...
namespace Mirror.Weaver
{
    ...
    public class Writers
    {
        ...
        MethodReference GenerateWriter(TypeReference variableReference, ref bool WeavingFailed)
        {
            ...
            {
                GenericInstanceType genericInstance = (GenericInstanceType)variableReference;
                TypeReference elementType1 = genericInstance.GenericArguments[0];
                TypeReference elementType2 = genericInstance.GenericArguments[1];

                return GenerateCollectionWriter(variableReference, elementType1, elementType2, nameof(NetworkWriterExtensions.WriteDictionary), ref WeavingFailed);
            }
        ...
        MethodDefinition GenerateCollectionWriter(TypeReference variable, TypeReference elementType1, TypeReference elementType2, string writerFunction, ref bool WeavingFailed)
        {

            MethodDefinition writerFunc = GenerateWriterFunc(variable);

            MethodReference elementWriteFunc1 = GetWriteFunc(elementType1, ref WeavingFailed);
            MethodReference elementWriteFunc2 = GetWriteFunc(elementType2, ref WeavingFailed);
            MethodReference intWriterFunc = GetWriteFunc(weaverTypes.Import<int>(), ref WeavingFailed);

            // need this null check till later PR when GetWriteFunc throws exception instead
            if (elementWriteFunc1 == null)
            {
                Log.Error($"Cannot generate writer for {variable}. Use a supported type or provide a custom writer", variable);
                WeavingFailed = true;
                return writerFunc;
            ...
            GenericInstanceMethod methodRef = new GenericInstanceMethod(collectionWriter);
            methodRef.GenericArguments.Add(elementType1);
            methodRef.GenericArguments.Add(elementType2);
