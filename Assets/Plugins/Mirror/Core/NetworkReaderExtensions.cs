...
namespace Mirror
{
    ...
    public static class NetworkReaderExtensions
    {
        ...
        */
        ...
        public static Dictionary<T0, T1> ReadDictionary<T0, T1>(this NetworkReader reader)
        {
            int count = reader.ReadInt();
            Dictionary<T0, T1> dictionary = new Dictionary<T0, T1>(count);
            for (int i = 0; i < count; ++i)
            {
                T0 key = reader.Read<T0>();
                T1 value = reader.Read<T1>();
